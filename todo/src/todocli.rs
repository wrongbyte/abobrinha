...
pub(crate) struct TodoCli {
    pub todo_controller: Box<dyn TodoController + Send + Sync>,
    ...
    pub async fn run(&mut self) -> Result<(), TerminalError> {
        loop {
            match self.user_interface.user_intention()? {
                ...
                UserOptions::NewTodo(todo) => self.todo_controller.add_todo(todo).await?,
                UserOptions::Help => self.user_interface.show_help()?,
                UserOptions::ClearList => self.todo_controller.clear_todo_list().await?,
                UserOptions::RemoveTodo(index) => self.todo_controller.remove_todo(index).await?,
                UserOptions::Unrecognized => self.user_interface.alert_unrecognized()?,
                UserOptions::ShowList => self.todo_controller.show_list().await?,
                UserOptions::DoTodo(index) => self.todo_controller.mark_todo_done(index).await?,
            }
        }
